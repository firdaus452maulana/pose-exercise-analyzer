...
class ProcessFrame:
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, lower_hips_disp):
        # print(c_frame)
        # print(np.where(c_frame))
        # print(np.where(c_frame)[0])
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                # Join landmarks.
                cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['white'], 4, lineType=self.linetype)
                cv2.line(frame, wrist_coord, elbow_coord, self.COLORS['white'], 4, lineType=self.linetype)
                cv2.line(frame, shldr_coord, hip_coord, self.COLORS['white'], 4, lineType=self.linetype)
                cv2.line(frame, knee_coord, hip_coord, self.COLORS['white'], 4, lineType=self.linetype)
                cv2.line(frame, ankle_coord, knee_coord, self.COLORS['white'], 4, lineType=self.linetype)
